...
if (window.HASEACH)//IFDEF
  ...
  test('map', function() {
    var doubled = _.map(function(num){ return num * 2; }, [1, 2, 3]);
    equal(doubled.join(', '), '2, 4, 6', 'doubled numbers');

    if (false){
        var ids = _.map(function(n){ return n.id; }, $('#map-test').children());
        deepEqual(ids, ['id1', 'id2'], 'Can use collection methods on jQuery Array-likes.');
    }
    if (false){
        var ids = _.map(function(n){ return n.id; }, document.images);
        ok(ids[0] == 'chart_image', 'can use collection methods on HTMLCollections');
    }
  });

  test('reduce', function() {
    var sum = _.reduce(function(sum, num){ return sum + num; }, 0, [1, 2, 3]);
    equal(sum, 6, 'can sum up an array');

    var prod = _.reduce(function(prod, num){ return prod * num; }, 1, [1, 2, 3, 4]);
    equal(prod, 24, 'can reduce via multiplication');
  });

  test('reduceRight', function() {
    var list = _.reduceRight(function(memo, str){ return memo + str; }, '', ["foo", "bar", "baz"]);
  ...
  test('find', function() {
    var array = [1, 2, 3, 4];
    strictEqual(_.find(function(n) { return n > 2; }, array), 3, 'should return first found `value`');
    strictEqual(_.find(function() { return false; }, array), void 0, 'should return `undefined` if `value` is not found');
  });

  test('detect', function() {
    var result = _.find(function(num){ return num * 2 == 4; },[1, 2, 3]);
    equal(result, 2, 'found the first "2" and broke the loop');
  });

  test('select', function() {
    var evens = _.filter(function(num){ return num % 2 == 0; },[1, 2, 3, 4, 5, 6]);
    equal(evens.join(', '), '2, 4, 6', 'selected each even number');

    evens = _.filter(function(num){ return num % 2 == 0; },[1, 2, 3, 4, 5, 6]);
    equal(evens.join(', '), '2, 4, 6', 'aliased as "filter"');
  });

  test('reject', function() {
    var odds = _.reject(function(num){ return num % 2 == 0; },[1, 2, 3, 4, 5, 6]);
    equal(odds.join(', '), '1, 3, 5', 'rejected each even number');
  });

  test('all', function() {
    ok(_.all(_.identity, []), 'the empty set');
    ok(_.all(_.identity, [true, true, true]), 'all true values');
    ok(!_.all(_.identity,[true, false, true]), 'one false value');
    ok(_.all(function(num){ return num % 2 == 0; }, [0, 10, 28]), 'even numbers');
    ok(!_.all(function(num){ return num % 2 == 0; }, [0, 11, 28]), 'an odd number');
